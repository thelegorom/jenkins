...
rm -f .repo/local_manifests/dyn-*.xml
repo init -u $SYNC_PROTO://github.com/thelegorom/latform_manifest.git -b $CORE_BRANCH $MANIFEST
...
if [ $TIME_SINCE_LAST_CLEAN -gt "21" -o $CLEAN = "true" ]
then
  echo "Cleaning!"
  touch .clean
  make clobber
...
time mka liquid recoveryzip recoveryimage checkapi
